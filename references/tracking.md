...
**Content note linkage:** If a content note exists in `outputs/content/` with a matching `content_id`, update its `status` to `published` and append to its `## Revision History` when logging.
